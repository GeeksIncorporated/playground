...
cursor = [0, 0]
COLS = ((200, 0, 0), (0, 200, 0))
FACTORS = (2, 3)
CENTER = (0, 0)
DISTS = [{}, {}]

def scale_point(x, y):
    return x // 20 + 600, y // 20 + 300
...
def go_right(n, wn):
    ...
    for i in range(1, n+1):
        M[y][x + i].append(wn)
        surface.set_at(scale_point(x + i, y), COLS[wn])
    x += n
    cursor = [x, y]
...
def go_up(n, wn):
    ...
    for i in range(1, n+1):
        M[y + i][x].append(wn)
        surface.set_at(scale_point(x, y + i), COLS[wn])
        # pygame.display.update()
    y += n
    cursor = [x, y]
...
def solve(w1, w2):
    ...
    min_d = (9999999,)
    min_steps = 9999999
    pygame.draw.circle(surface, (0, 200, 200), scale_point(0, 0), 3)
    for x in M:
        for y in M[x]:
            r = len(set(list(M[x][y])))
            if r > 1 and (x, y) != (0, 0):
                print(min_d)
                min_d = (min(min_d[0],
                             abs(x) + abs(y)), x, y)
                # min_steps = min(
                #     min_steps, DISTS[0][(y, x)] + DISTS[1][(y, x)])
                # print(min_steps)
                pygame.draw.circle(surface, (200, 200, 200), scale_point(y, x), 2)
                # pygame.draw.line(surface, (200, 200, 0), scale_point(0,0), scale_point(y, x), 1)
                pygame.display.update()
                time.sleep(0.05)
    time.sleep(5)
    print((min_d, min_steps))
